...
class Board {
  ...
  private:
    ...
    vector<int> setsize;        // Vector used to store set sizes
...
void Board::initSets() {
  ...
  setsize.resize(allNodes.size(),1);
  for (auto i:allNodes)
...
void Board::combineSet(const int &u, const int &v) {
  int uSet = findSet(u), vSet = findSet(v);
  if (uSet == vSet)
    return;
  if (setsize[uSet] < setsize[vSet]) {
    set[uSet] = vSet;
    setsize[vSet] += setsize[uSet];
  } else {
    set[vSet] = uSet;
    setsize[uSet] += setsize[vSet];
  }
}
...
// Return the root of u set 
int Board::findSet(const int &u) const {
  int root = u;
  while (root != set[root])
    root = set[root];
  return root;
