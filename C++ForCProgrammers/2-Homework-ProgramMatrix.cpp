...
// Initializes number of nodes and initial edge values
Graph::Graph(int numVertices, int initialValue=INFINIT)
{
  ...
  // Create adjacency matrix with all nodes and initialize edges
  adjMatrix.resize(numVertices*numVertices, initialValue);
}
...
{
  return adjMatrix[nodeNumbers[x]*numV + nodeNumbers[y]];
}

// Set edge weight between 'x' and 'y'
void Graph::set_edge_value(char x, char y, int value)
{
  if (adjMatrix[nodeNumbers[x]*numV + nodeNumbers[y]] == INFINIT)
    ++numE;
  adjMatrix[nodeNumbers[x]*numV + nodeNumbers[y]] = adjMatrix[nodeNumbers[y]*numV + nodeNumbers[x]] = value;
}

// Return true if 'x' and 'y' are neighbors and false otherwise
bool Graph::adjacent(char x, char y)
{
  return (adjMatrix[nodeNumbers[x]*numV + nodeNumbers[y]] != INFINIT);
}
...
list<char> Graph::neighbors(char x)
{
  ...
  for (int y=0; y<numV;++y)
    if (adjMatrix[nodeNumbers[x]*numV + y] != INFINIT)
      adjNodes.push_back(nodeNames[y]);
  return adjNodes;
}
...
list<char> Graph::vertices()
{
  list<char> nodes;
  for (int x=0; x<numV;++x)
    nodes.push_back(nodeNames[x]);
  return nodes;
}

// Print out adjacency matrix representing the Graph
void Graph::show()
{
  cout << "  ";
  for (int y=0; y<numV;++y)
